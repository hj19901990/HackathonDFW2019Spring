...
#include "Com.h"
	...
	bool CComm::Listen(int PortNum)
	{
		...
		ListenSocket = socket(PF_INET, /*SOCK_STREAM*/SOCK_DGRAM, 0);
		if (ListenSocket == INVALID_SOCKET)
		{
		...
		srv.sin_addr.S_un.S_addr = htonl(INADDR_ANY);
		//srv.sin_addr.s_addr = htonl(INADDR_ANY);	// Any addressinet_addr("127.0.0.1");
